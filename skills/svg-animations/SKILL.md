...
This skill owns the SVG asset itself. Page-level design direction belongs to `hallmark` / `frontend-design`; article diagrams and image specs belong to `technical-writer` — they may all call into this skill for the animation work.

## Workflow

1. **Decide the animation technology first**, from how the SVG will be delivered:
   - Loaded via `<img>` / CSS `background-image` → **SMIL** (CSS/JS can't reach inside).
   - Inlined in the page and coordinated with page styles/state → **CSS** (or JS for dynamic values).
   - Interactive morphs on click/hover inside an `<img>` → SMIL `begin="click"`; inlined → CSS/JS.
2. **Build static first.** Get the shapes, viewBox, and composition right before adding motion — animation hides geometry mistakes until they're expensive.
3. **Animate**, using the patterns and recipes below.
4. **Verify before delivering:**
   - Renders correctly at small and large sizes (viewBox only, no fixed width/height).
   - Morph/`d` animations interpolate (identical command count, types, and order between states).
   - `<title>`/`<desc>` + `role="img"` present.
   - Reduced-motion behavior defined (see the requirement below).
   - Looping animations are subtle enough to live on a page indefinitely without distracting.

## Output contract

Deliver a complete, self-contained `.svg` (or inline snippet with its `<style>` block), plus one line each on: chosen technology and why, how to embed it, and what the reduced-motion fallback shows.

**Reduced-motion is a requirement, not a nicety.** Every animated SVG ships with a `prefers-reduced-motion` story: for CSS animation, the media-query override (Best Practice 12); for SMIL inside an `<img>`, there is no media-query hook — state the static fallback (e.g. serve a static variant or accept the first frame) rather than silently ignoring it.
