...
| `forge` | Drive one issue (Linear, GitHub, or in-repo docs) to a PR and loop review→rectify until clean; stops before merge. `--hitl` splits the work into an approved plan, waits for feedback after each tranche, and gates every review fix on an agreed resolution and a sign-off before commit |
