 ...
 * them (Tailwind's scanner cannot see dynamically-built class names). The
 * palette is defined in styles.css (`--tone-*` variables, exposed as Tailwind
 * colors). If tokens are stored in a DB, the names must never change.
 */
export type SemanticColor =
  | "primary" | "secondary" | "accent" | "info" | "success"
  | "warning" | "error" | "neutral" | "ghost";
