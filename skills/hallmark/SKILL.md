---
name: hallmark
description: "Anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build a new app or landing page, wants to redesign something, invokes Hallmark by name, or uses audit/redesign/study. For UI work inside an established Astro + DaisyUI design system, prefer the frontend-design skill instead."
version: 1.0.0
---
...
**Routing vs. `frontend-design`:** Hallmark owns aesthetic-first builds where the design direction is the deliverable. When the task is UI work inside an established stack-constrained design system (this library's `frontend-design` skill: Astro + DaisyUI semantic tokens), that skill owns it — don't impose a Hallmark theme on top of a token system the team already maintains.
...
| `hallmark redesign <target> [--mood <name>]` | Take the target's content and intent, then redesign the visual structure **inside the existing implementation boundaries unless the user explicitly confirms a full rebuild.** New section rhythm, new heading placement, new component voice. Preserve existing routes, component ownership, copy intent, brand, and information architecture; replace only the visual/interaction layer needed for the requested scope. **Always show a rendered before/after sample of one representative section and wait for explicit confirmation before writing to any production file** — see the before/after preview gate in [`verbs/redesign.md`](references/verbs/redesign.md). |
...
- **On `redesign`, the bar is higher than stating files: render a before/after sample of one representative section, show it alongside a structural before→after diff, and wait for explicit "go ahead" before writing anything.** A redesign swaps out something the user already shipped — they confirm the *trade*, having seen both sides, not just a description of the new design. See [`verbs/redesign.md`](references/verbs/redesign.md) § Before/after preview gate.
...
The per-theme axis values live in [`references/theme-axes.md`](references/theme-axes.md) — the canonical table for all 22 catalog themes. Look values up there rather than inferring them from the theme name. When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
...
The user has supplied a reference — an attached screenshot or a URL to a live page — of a design they admire. They want its *DNA* (shape, type, rhythm) applied to their own content, not a pixel-faithful copy.

**Critical position:** `study` extracts structure, not pixels. It produces a *diagnosis report* before any code, then offers to rebuild the user's content using the extracted DNA. Pixel-cloning is not a feature.

**Load [`references/study.md`](references/study.md) and follow it.** That file is the full protocol: source-mode detection (URL prefix → URL mode, otherwise image mode), the refusal heuristics and URL refuse list (run **before** any WebFetch), the extraction passes, the structured-fields schema, the diagnosis-report templates, the junk-or-blocked URL fallback, the confirmation question and its three branches (build with the DNA · lock the DNA into a `design.md`, with URL-mode attestation · stop at the diagnosis), the `studied: yes` output-contract stamps, and the limits to spell out to the user (font-ID reliability, no imagery copying, theme drift allowed, the URL-mode rhythm blind spot). Do not work from intuition.
